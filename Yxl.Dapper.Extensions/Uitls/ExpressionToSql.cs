...
namespace Yxl.Dapper.Extensions.Uitls
{
    internal class ExpressionToSql
    {
        ...
        private string BinaryExpression(Expression expression)
        {
            ...
            var left = Recurse(body.Left, leftisUnary);

            var columnName = left.Replace("{", "").Replace("}", "").
                Replace("[", "").Replace("]", "").Replace("(", "").Replace(")", "");
            var parameterValue = right.Replace("{", "").Replace("}", "").Replace("'", "");
            if (columnName.Contains("=") == false && columnName.Contains("<") == false &&
                columnName.Contains(">") == false && parameterValue != Guid.Empty.ToString() && string.IsNullOrEmpty(parameterValue) == false &&
                string.IsNullOrWhiteSpace(parameterValue) == false)
            {
                Parameter[columnName] = parameterValue;
            }
        ...
        private string MemberExpression(Expression expression, bool isUnary = false, bool quote = true)
        {
            ...
            {
                var colName = member.Member.GetCustomAttribute<ColumnAttribute>()?.Name ?? member.Member.Name;
        ...
        private string ValueToString(object value, bool isUnary, bool quote)
        {
            ...
            else if ((value is System.Enum))
            {
                ...
                var enumValue = Convert.ToInt16(System.Enum.Parse(enumType, value.ToString()));
                return enumValue.ToString();
            }
            else if ((value is DateTime) && quote)
            {
        ...
        private static object NodeTypeToString(ExpressionType nodeType, bool rightIsNull)
        {
            ...
            throw new NotSupportedException($"Unsupported nodeType: {nodeType}");
        }
    }
}
